...
public class Conducteur implements Interface{
    ...
    public boolean AlreadyExist(Connection connexion) throws SQLException {
        PreparedStatement preparedStatement = connexion.prepareStatement("SELECT * FROM `driver` WHERE prenom = ? AND nom = ? ");
    ...
    public void Insertion(Connection connexion) throws SQLException {
        if(!this.AlreadyExist(connexion)) {
            PreparedStatement preparedStatement = connexion.prepareStatement("INSERT INTO `driver` (`prenom`, `nom`) VALUES (?, ?);");
            preparedStatement.setString(1, this.Prenom);
            preparedStatement.setString(2, this.Nom);
